...
pv, pv_available = utils.try_import_pyvista()       # pyvista库
vtk, nps, vtk_available = utils.try_import_vtk()    # VTK库
...
class Volume(object):
    ...
    data: 密度数据（3D数组）
    materials: CT容积物质分割
    anatomical_from_ijk: 从IJK空间到解剖学空间（RAS/LPS）的变换
    world_from_anatomical: 从解剖学空间到世界坐标的变换
    anatomical_coordinate_systems: 指定解剖学空间的字符串，"LPS"，"RAS"或None，Optional表示参数可选
    """
    data: np.ndarray
    ...
    def _segment_materials(
        ...
        """
        if use_thresholding:
            print('using threshold to segment_in class')
            return load_dicom.conv_hu_to_materials_thresholding(hu_values)    # 阈值分割
        else:
            print('using DNN to segment_in class')
            return load_dicom.conv_hu_to_materials(hu_values)                 # DNN方法分割
    ...
    def from_nifti(
        ...
        **kwargs,
        ...
        if cache_dir is None:
            ...
            cache_dir = os.path.join(cache_dir, 'temp')
